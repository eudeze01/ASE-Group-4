...
function viewBookedVehicleData(v_plate, model, type, driver_name, rating) {
  ...
  num.innerText = (rating * 5).toFixed(2) + "/5";
...
function calculateFair(dist = 0) {
  const now_t = new Date();
  // Time surcharges :
  // morning 6am - evening 5pm no surcharge
  // 5pm - 10pm 		5 pounds
  // 10pm - 6am 		6 pounds

  const surcharge =
    now_t.getHours() >= 0 && now_t <= 5
      ? 6
      : now_t.getHours() >= 6 && now_t.getHours() <= 16
      ? 0
      : now_t.getHours() >= 17 && now_t.getHours() <= 21
      ? 5
      : 6;

  // 1st 5 Km - 1.20 pounds
  // next every 1Km - 1 pound

  const amount = dist > 5 ? 6 + (dist - 5) : dist * 1.2;
  ...
  start_jny.addEventListener("click", () => {
    if (!journeyStarted) {
      ...
      const options = {
        ...
        body: JSON.stringify({ fair: journey_fair }),
      };

      fetch("/api/endJourney", options).then((res) => {
        if (res.ok) {
          journeyEnded = true;
          updateUI();
        }
